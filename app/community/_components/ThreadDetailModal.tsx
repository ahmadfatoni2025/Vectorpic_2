...
import {
  ...
  MoreHorizontal, Image as ImageIcon, Smile, ArrowLeft, CheckCircle2
} from 'lucide-react';
...
export const ThreadDetailModal = ({ thread, isOpen, onClose, sessionId }: ThreadDetailModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Ensure the body doesn't jump by potentially accounting for scrollbar width if needed
      // (Optional simple fix: body { padding-right: scrollbar-width })
  ...
  useEffect(() => {
    ...
    if (!isOpen) {
      lastFetchedRef.current = null; // Reset when closed so next open re-fetches
    }
  }, [isOpen, thread?.id]);
      ...
      {isOpen && thread && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center pointer-events-none">
          ...
          <motion.div
            ...
            className="relative w-full max-w-4xl bg-white flex flex-col h-[94vh] shadow-[0_-20px_60px_-15px_rgba(0,0,0,0.4)] rounded-t-[40px] pointer-events-auto overflow-hidden border-x border-t border-gray-100"
          >
            ...
            {/* Header */}
            <div className="flex items-center gap-4 px-8 py-5 border-b border-gray-50 flex-shrink-0">
              ...
              <div className="p-8 pb-10 bg-white">
                ...
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[16px] font-black text-gray-900">{thread.user.name}</span>
                ...
                {/* Body */}
                <h3 className="text-[24px] font-black text-gray-900 leading-[1.1] mb-5 tracking-tight border-b border-gray-50 pb-5">
                  {thread.title}
                </h3>
                <p className="text-[17px] text-gray-700 leading-relaxed font-medium mb-8 whitespace-pre-wrap">
                ...
                <div className="flex items-center justify-between pt-8 border-t border-gray-50">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={handleBookmark} className={`p-3 rounded-2xl transition-all ${isBookmarked ? 'bg-[#FF4D00] text-white shadow-lg shadow-orange-100' : 'bg-gray-50 text-gray-400 border border-gray-100'}`}>
                      <Bookmark size={20} strokeWidth={2.5} className={isBookmarked ? "fill-current" : ""} />
                    </button>
                    <button onClick={handleShare} className="p-3 bg-gray-50 text-gray-400 rounded-2xl border border-gray-100 transition-all hover:bg-gray-100">
              ...
              <div className="p-8 pt-4 space-y-6">
                ...
                {loadingComments ? (
                  ...
                  <div className="py-20 text-center bg-white rounded-[32px] border border-dashed border-gray-200">
                    ...
                    <h5 className="text-[15px] font-black text-gray-900 mb-2">No replies yet</h5>
                    <p className="text-[13px] text-gray-400 font-medium">Be the first to share your perspective!</p>
                  </div>
                ) : (
                        ...
                        <div className="flex gap-5">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-[14px] font-black text-gray-900">{comment.author?.name || comment.guestName || 'Guest'}</span>
                              <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest">{formatTime(comment.createdAt)}</span>
                            </div>
                            <p className="text-[15px] text-gray-600 leading-relaxed font-bold break-words whitespace-pre-wrap">
                            ...
                            <div className="flex items-center gap-6 mt-4">
                              <button className="flex items-center gap-1.5 text-gray-400 hover:text-[#FF4D00] transition-colors group/clike">
                                ...
                                <span className="text-[11px] font-black uppercase tracking-widest">{comment.likesCount || 0}</span>
                              </button>
                              <button className="text-[11px] font-black text-gray-300 hover:text-gray-900 uppercase tracking-widest transition-colors">
                                Reply
                              </button>
                            </div>
                          </div>
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className="w-full bg-gray-50 border border-transparent rounded-[24px] px-6 py-4 text-[15px] font-medium text-gray-800 placeholder:text-gray-300 outline-none focus:bg-white focus:border-[#FF4D00] transition-all resize-none min-h-[64px]"
                  />
                  <div className="absolute right-3 bottom-2 flex items-center gap-1">
                    <button
                      onClick={handleSubmitComment}
                      disabled={!newComment.trim() || submitting}
                      className="w-10 h-10 bg-[#FF4D00] text-white rounded-2xl flex items-center justify-center shadow-lg shadow-orange-100 disabled:opacity-30 transition-all hover:bg-[#E64500]"
                    >
                      {submitting ? (
