      ...
      >
        ...
        <form
          ...
          onSubmit={async (e) => {
            ...
            const finalUploadData = {
              ...uploadData,
              categoryId: uploadData.categoryId ? Number(uploadData.categoryId) : null
            };

            if (!finalUploadData.title || !finalUploadData.imageUrl || !finalUploadData.categoryId) {
              return;
            }

            const success = await onAction('POST', '/api/vectors', finalUploadData);
            if (success) {
        ...
        >
          ...
          <div className="px-8 py-6 border-t border-gray-50 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={!uploadData.title || !uploadData.imageUrl || !uploadData.categoryId}
              className="px-5 py-2.5 bg-black text-white text-[14px] font-bold rounded-xl hover:bg-gray-900 transition-all active:scale-95 shadow-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
